document.querySelector('.siguiente').addEventListener('click', () => {
    ...
    let verificar = Number(localStorage.getItem('guardar-Level23-JS')) || 0;
    if (verificar === 1){
        window.location.href = '../JS-51/index.html';
    } else if (verificar === 0){
        if (input1 === '.appendChild') {
            alert("Felicitaciones, has completado el Nivel 23!");
            localStorage.setItem('guardar-Level23-JS', 1);
            window.location.href = '../JS-51/index.html';
        } else if (input1 === "") {
