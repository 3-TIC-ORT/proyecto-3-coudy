document.querySelector('.siguiente').addEventListener('click', () => {
    ...
    let verificar = Number(localStorage.getItem('guardar-Level27-JS')) || 0;
    if (verificar === 1){
        window.location.href = '../JS-62/index.html';
    } else if (verificar === 0){
        if (input1 === 'frutas.push') {
            alert("Felicitaciones, has completado el Nivel 27!");
            localStorage.setItem('guardar-Level27-JS', 1);
            window.location.href = '../JS-62/index.html';
        } else if (input1 === "") {
