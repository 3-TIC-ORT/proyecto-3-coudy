document.querySelector('.siguiente').addEventListener('click', () => {
    ...
    let verificar = Number(localStorage.getItem('guardar-Level21-JS')) || 0;
    if (verificar === 1){
        window.location.href = '../JS-47/index.html';
    } else if (verificar === 0){
        if (input1 === '"Texto cambiado"') {
            alert("Felicitaciones, has completado el Nivel 21!");
            localStorage.setItem('guardar-Level21-JS', 1);
            window.location.href = '../JS-47/index.html';
        } else if (input1 === "") {
